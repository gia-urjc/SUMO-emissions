...
def closeToRestrictedArea(veh):
    string_current_edge = traci.vehicle.getRoadID(veh.id) + "_0"
    if string_current_edge in enter_control_area_edges:
        dist = traci.vehicle.getLanePosition(veh.id) # TODO ALGO DE AQUÍ DE WARNING: ARREGLAR, warning: Request backPos of vehicle 'veh676' for invalid lane ':gneJ94_0_0' time=2769.00.
...
def all_cars_enter(simulation):
    for veh in simulation.vehicles_in_simulation:
        if closeToRestrictedArea(veh) :
            if traci.vehicle.getVehicleClass(veh.id) == "custom1":
                setAllowCar(veh)

def no_cars_enter(simulation):
    for veh in simulation.vehicles_in_simulation:
        if closeToRestrictedArea(veh) :
            if traci.vehicle.getVehicleClass(veh.id) != "custom1":
                setNotAllowCar(veh)

def some_cars_enter(simulation):
    for veh in simulation.vehicles_in_simulation:
        if closeToRestrictedArea(veh) :
            ...
            try:
                ...
                elif strategy == "VE" or strategy == "VEP" or strategy == "RRE" or strategy == "RREP":
...
def historicalTester (simk,veh):
    ...
    if strategy == "VEP" or strategy == "RREP":
