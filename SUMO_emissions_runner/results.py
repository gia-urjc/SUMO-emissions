...
def results(simulation, window_size, p_t_ini, size_ratio, subs_NOx, e_ini, min_packages, max_packages, route = ""):
    ...
    with open(fileName, mode='w', newline='') as f_csv:
        ...
        f.writerow(["window_size", "threshold_L", "threshold_H", "p_t_ini", "e_ini", "size_ratio", "subs_NOx", "min_packages", "max_packages"])
        ...
        f.writerow(["total_steps(sec)", "minutes", "avg_time_per_package (sec)", "p_t_total_all_steps",
                    ...
                    "avg_k_all_steps", "veh_enter_cz_all_steps", "not_veh_enter_cz_all_steps", "avg_total_time_all_steps"])
        ...
        for w in simulation.windows:
            ...
            f.writerow([str(w.step), str(w.NOx_total_w), str(acum), str(w.lambda_l), str(w.p_t_total),
                        str(w.NOx_total_w), str(w.p_t), str(w.NOx_control_zone_w), str(w.k), str(w.veh_total_number_w),
        ...
        for v in simulation.all_veh:
            ...
            f.writerow([v.id, v.vType, str(v.NOx), str(v.n_packages), str(v.step_ini), str(v.step_fin), str(total_time),
